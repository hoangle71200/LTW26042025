...
import CreateLoaiSPModal from './CreateLoaiSPModal'
import UpdateLoaiSPModal from './UpdateLoaiSPModal'
...
const DanhMucLoaiSP = () => {
  ...
  const handleDelete = async () => {
    for (let i = 0; i < selectedCheckboxes.length; i++) {
      await DanhMucLoaiSPSer.delete(selectedCheckboxes[i])
    }
  ...
  return (
    <div>
      <div>
        <HeaderPage title={"Danh mục loại sản phẩm"}/>
        <Button onClick={() => setShowModal(true)}>Thêm loại sản phẩm</Button>

        <h3 style={{ textAlign: 'center' }}>Danh sách loại sản phẩm</h3>
        ...
        <table className='table table-striped' style={{ width: '100vw' }}>
          ...
          {listPr.map((item, index) => (
            <tr key={index}>
              ...
              <td>{(item.productDepartment == null) ? "" : item.productDepartment.name}</td>
              <td>{item.note}</td>
              <td>
      ...
      <UpdateLoaiSPModal open={showModalUpdate} handleClose={handleCloseModalUpdate}
                         updateItem={updateItem} listNganhHang={listNganhHang}/>
    </div>
  )
}

export default DanhMucLoaiSP;
