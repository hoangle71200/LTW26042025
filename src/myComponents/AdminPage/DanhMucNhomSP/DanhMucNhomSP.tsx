...
import CreateNhomSPModal from './CreateNhomSPModal'
import UpdateNhomSPModal from './UpdateNhomSPModal'
...
const DanhMucNhomSP = () => {
  ...
  const handleDelete = async () => {
    for (let i = 0; i < selectedCheckboxes.length; i++) {
      await DanhMucNhomSPSer.delete(selectedCheckboxes[i])
    }
  ...
  return (
    <div>
      <div>
        <HeaderPage title={"Danh mục nhóm sản phẩm"}/>
        <Button onClick={() => setShowModal(true)}>Thêm nhóm sản phẩm</Button>

        <h3 style={{ textAlign: 'center' }}>Danh sách nhóm sản phẩm</h3>
      ...
      <CreateNhomSPModal open={showModal} handleClose={handleCloseModal} listLoaiSP={listLoaiSP}/>
      <UpdateNhomSPModal open={showModalUpdate} handleClose={handleCloseModalUpdate} updateItem={updateItem} listLoaiSP={listLoaiSP} />
    </div>
  )
}

export default DanhMucNhomSP;
